...
def start(request):
    with open('paraplaner.pythonanywhere.com/MSP/universities.json', 'r', encoding='utf-8') as js:
...
def tasks(request):
    ...
    start_date = Day.objects.get(date=settings.start_date,owner=user)
    end_date = Day.objects.get(date=settings.end_date,owner=user)
    template='MSP/tasks.html'
    user_agent = request.META['HTTP_USER_AGENT']
    if 'Mobile' in user_agent:
        template='MSP/tasks_mobile.html'
    return render(request, template,
...
def task_new(request):
    ...
    start_date = Day.objects.get(date=settings.start_date,owner=user)
    end_date = Day.objects.get(date=settings.end_date,owner=user)
    template='MSP/task_edit.html'
    user_agent = request.META['HTTP_USER_AGENT']
    if 'Mobile' in user_agent:
        template='MSP/task_edit_mobile.html'
...
def task_new_lesson(request, lesson, date, time, menu, info):
    ...
    start_date = Day.objects.get(date=settings.start_date,owner=user)
    end_date = Day.objects.get(date=settings.end_date,owner=user)
    template = 'MSP/task_edit_lesson.html'
    user_agent = request.META['HTTP_USER_AGENT']
    if 'Mobile' in user_agent:
        template = 'MSP/task_edit_lesson_mobile.html'
        form = TaskMobileForm()
    return render(request, template,
